...
\node[color=black] at ($(current page.north west)-(-7.1*\safetymargin,\headerheight)$){%
...
\node [color=black!65,above left,inner sep=0pt] at ($(current page.south east)+(-\footermargin,\footerheight)+(0pt,.25\footermargin)$) {%
  \fontsize{20}{35}\selectfont%
  Theme based on Matt Graham's \emph{Leap Day}.};
...
{Chapter activities}
  ...
  \begin{itemize}
    ...
    \item Student {\color{LDblue}Krylov Day} at TU Delft,
    \item Learning-by-doing sessions on software tools [ba{\color{LDblue}NaN}a talks],
    \item {\color{LDblue}Company visits} and {\color{LDblue}socials events} like faculty BBQ, bowling, \dots
   \end{itemize}
...
{}
 \includegraphics[height=9.5cm]{library1} \hfill
 ...
 \includegraphics[height=9.5cm]{whiteboard1}
\end{textbox}
...
{SIAM chapters in Europe}
  ...
  \begin{center}
    The {\color{LDblue}first} SIAM student chapter in The Netherlands is installed at TU Delft.
  \end{center}
